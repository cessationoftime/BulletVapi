...
PlPhysicsSdk*	physicsSdk=0;
PlDynamicsWorld*	dynamicsWorld=0;
PlRigidBody* floorRigidBody;
PlRigidBody* dinoRigidBody;
...

{
  ...
  /* Translate the dinosaur to be at (0,8,0). */
	...
	pl_rigidbody_GetOpenGLMatrix(dinoRigidBody,matrix);
//	pl_rigidbody_GetPosition(dinoRigidBody,dinoWorldPos);
...
idle(void)
{
  ...
  if (dynamicsWorld)
		pl_dynamicsworld_StepSimulation(dynamicsWorld,dtime);
...
{
  int i;
	PlCollisionShape* floorShape;
	PlCollisionShape* dinoShape;
	PlCollisionShape* dinoChildShape;
	PlVector3* floorPos = pl_vector3_new();
	PlVector3* childPos = pl_vector3_new();
	PlVector3* dinoPos = pl_vector3_new();
	PlQuaternion* childOrn = pl_quaternion_new();
	PlQuaternion* dinoOrient = pl_quaternion_new();
	
	void* user_data=NULL;
	
	physicsSdk = pl_physicssdk_new();
	dynamicsWorld = pl_dynamicsworld_new(physicsSdk);

	//create ground plane

	floorShape = pl_convexhullshape_new();
	
	for (i=0;i<4;i++)
	{
		pl_convexhullshape_AddVertex(floorShape,floorVertices[i][0],floorVertices[i][1],floorVertices[i][2]);
	}
	

	floorShape = pl_boxshape_new(120,0,120);

	floorRigidBody = pl_rigidbody_new(user_data,0.f,floorShape);

	floorPos[0] = 0;
	floorPos[1] = 0;
	floorPos[2] = 0;

	pl_rigidbody_SetPosition(floorRigidBody,floorPos);
	pl_dynamicsworld_AddRigidBody(dynamicsWorld,floorRigidBody);

	//create dino rigidbody
	dinoChildShape = pl_boxshape_new(8.5,8.5,8.5);
	dinoShape = pl_compoundshape_new();
	childPos[0] = 0;
	childPos[1] = 0;
	childPos[2] = 0;
	childOrn[0] = 0;
	...
	pl_compoundshape_AddChildShape(dinoShape,dinoChildShape,childPos,childOrn);

	dinoPos[0] = -10; dinoPos[1] = 28; dinoPos[2] = 0;
	dinoRigidBody = pl_rigidbody_new(0,1.0,dinoShape);
	pl_rigidbody_SetPosition(dinoRigidBody,dinoPos);

	pl_SetEuler(0,0,3.15*0.20,dinoOrient);
	pl_rigidbody_SetOrientation(dinoRigidBody,dinoOrient);

	pl_dynamicsworld_AddRigidBody(dynamicsWorld,dinoRigidBody);
...
// Register GLUT callbacks. 
  ...
  pl_dynamicsworld_free(dynamicsWorld);
  pl_physicssdk_free(physicsSdk);
